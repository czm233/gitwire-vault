...
> 全景扫描：2026-09-25 · 开放 issue 93 个 · 机会 47 · 被占 27 · 困难 19

## 本期机会（无人认领、无 PR、难度 ≤ 中等）
- **#1584** [中等] Aggregate cost SUMs (sessions/nodes) silently drop unresolved-cost calls, unlike the fixed row-level endpoint —— 聚合SUM掩盖未定价调用的null成本，与已修的单行端点行为不一致，站点明确
- **#1582** [中等] Heterogeneous tuples do not constrain element position —— 异构元组需改用prefixItems按位约束，修法明确但涉及schema版本兼容
- **#1581** [中等] Optional[X] with no default is reported as not required —— Union处理错误放宽required，修复明确且影响函数调用正确性
- **#1577** [简单] docs: web search tool page doesn't document installing the `websearch` extra —— websearch 文档页缺 optional extra 安装说明，纯文档补全
- **#1574** [中等] Session flow_name warning fires on railtracks' own internal call paths —— 内部调用路径的 Session 触发 flow_name 警告，误扰正常用户
- **#1573** [简单] railtracks add --force before <tool>:<skill> fails, despite usage text showing that order —— `railtracks add --force` 前置位置解析顺序 bug，定位明确的 CLI 修复
- **#1534** [简单] Delete legacy install detection —— 删除 legacy 安装检测代码，前置条件已满足，范围清晰的清理任务
- **#1509** [中等] Args used to throw a new error is not clear. —— 统一错误类的 reason/message 术语并上提 notes+reason 到 RTError，需设计决策
- **#1503** [中等] Optional markdown render on visualizer —— visualizer 增加 AI 输入/输出的 markdown raw/preview 切换，范围明确的前端增强
- **#1496** [中等] Let input guards fire once per agent call instead of per model round-trip —— 给 InputGuard 加 once 开关避免每次工具循环重复触发，方案已明确
- **#1468** [中等] MessageHistory serializable and loadable —— MessageHistory/消息对象支持 JSON 序列化与反序列化加载
- **#1463** [中等] Long file cleanup: _litellm_wrapper.py —— 重构超长文件 _litellm_wrapper.py 及测试，抽公共逻辑
- **#1462** [简单] Logger: RuntimeError on  unretrieved-task —— publisher 关闭后任务仍发布导致 RuntimeError，需条件保护
- **#1446** [中等] Context Events —— 新增 context 创建/读写/完成四类可观测事件
- **#1435** [中等] Railtrack should not modify litellm behaviour globally —— railtracks 导入时全局修改 litellm 参数，应局部化
- **#1415** [中等] Event emission after forced exit —— 为 ctrl+c 中断注册信号处理器，发送终止事件并刷新
- **#1413** [中等] Migrate Current Colourful Logging to use Events Approach —— 彩色日志迁移到事件驱动方案，需设计验证
- **#1409** [中等] Cost Limiting Middleware —— 成本限额中间件，设计已有雏形的功能
- **#1398** [中等] post-init assignment guard/docmentation —— post-init 赋值守卫，涉及大量字段迁移、规范类任务
- **#1395** [中等] Provide even further tool abstraction —— 为 agent-as-tool 类场景增加 Custom Tools 抽象
- **#1381** [简单] Add llms.txt / llms-full.txt —— 添加 llms.txt / llms-full.txt，范围明确的文档任务
- **#1375** [中等] [Documentation] Doc page content and naming duplications —— 清理文档页重复与命名冲突，量大但机械
- **#1356** [中等] Non-Uniform Provider Initialization & Model String Mapping —— 统一 LLM 子类初始化接口与 from_string 工厂
- **#1354** [中等] Token estimator —— 通用 token 估算器，范围清晰但需选型设计
- **#1353** [中等] Serializable transcript / history export/input from popular formats —— OpenAI 格式无损导入导出 API，范围明确
- **#1349** [中等] Non-image attachments on UserMessage (audio, video, file) —— Attachment 支持音频/视频/文件模态，单模块功能扩展
- **#1320** [中等] Attachment location interleaving —— 附件在消息中的位置交错，媒体块位置信息丢失
- **#1316** [简单] Improve instruction for using custom function as tool —— 改进自定义函数作为工具的文档说明
- **#1306** [中等] Build a Guardrail for Work Scope —— ScopeGuard 护栏拦截超出范围的提问
- **#1251** [中等] viz command: formalize --dir flag and add subdirectory awareness for custom data paths —— viz 命令 --dir 标志形式化并修复子目录感知 bug
- **#1248** [中等] Create a ModelRequest type —— 引入 ModelRequest 类型统一模型调用入参
- **#1223** [简单] [Feature] Prebuilt Tool Tags —— 给预构建工具打标签以便可视化器区分展示
- **#1209** [中等] [Tech Debt] Current Pgvector Backend throws all metadata to a single payload dict —— pgvector 后端把所有 metadata 塞单 payload 列
- **#1203** [中等] [Feature] Thoughts Tool —— 新增 Thoughts 工具：脑洞/键值/向量三种暂存草稿
- **#1186** [中等] [Perf] [Retrieval] Chroma backend stores chunk content twice (documents + metadata payload) —— Chroma 后端 chunk 内容双份存储，浪费成本
- **#1185** [中等] [Bug] [Retrieval] User chunk metadata can overwrite reserved payload keys (content, document_id, ...) —— 用户 metadata 可覆盖保留 payload 键导致数据损坏
- **#1184** [中等] [Perf] [Retrieval] Ingest issues one store round-trip per chunk — add bulk writes —— ingest 逐 chunk 单次写库，需增加批量写接口
- **#1173** [中等] [Feature] [Retrieval] Add `Turbovec` to our stores Integration —— 新增 Turbovec 向量后端集成
- **#1172** [中等] [Feature]  Define certain class as internal —— 将 _Session 等内部类标记为 internal，防误用
- **#1167** [简单] [Bug] [Retrieval] same `id` for all rows in `HuggingFaceDatasetLoader` —— HF 数据集所有行 id 相同，破坏 upsert 语义
- **#1160** [中等] [Docs] Prefer if api_reference lives under main doc site —— 把 pdoc API 参考嵌入主文档站，提升一致性
- **#1150** [简单] [Feature] [Retrieval] Batch write for stores —— 存储层批量写入 EmbeddedChunk，接口扩展明确
- **#1065** [中等] [Feature] `retry` mechanism for blocked guardrail decisions —— 被拦截护栏的重试机制，需设计决策
- **#1053** [简单] [Feature] Add Length Requirement Guardrail —— 长度护栏 Guardrail，接口已定范围明确
- **#1014** [中等] [Feature] Evaluation Shots/Examples/Rubric —— 评估指标增加 shots/examples 支持，DX 设计待定
- **#940** [简单] [Docs] Update BYFA/RYFA to be a bit better in terms of narrative structure —— BYFA/RYFA 教程叙事重构，纯文档改写
- **#873** [简单] [Docs] Unclear Usage of Context —— Context 用法文档前置，纯文档改动，适合入门

## 已被占（不必再看）
- #1562 Tool.from_function silently degrades unmapped parameter types to "object" (bypasses #1552's strict validation) —— 被 rajathpatel23 占
- #1547 Remove/soften model specific instruction in docs —— 被 PR#1583 占
- #1538 Type hints collapse when you use a list of pre-built middlewares —— 被 PR#1558 占
- #1474 We need a ticket assign max duration or PR open duration —— 被 CoronRing 占
- #1458 Tool schemas silently degrade to `{"type": "object"}` under `from __future__ import annotations` —— 被 CoronRing 占
- #1451 Skill Infrastructure: Directories Support —— 被 Pooria90 占
- #1433 Precise request cost with cache hit info —— 被 PR#1519 占
- #1431 Surface thinking token in LLM response. —— 被 Pooria90 占
- #1430 LLM finish_reason  is not respected —— 被 CoronRing 占
- #1425 Emit verifier decisions as observability events —— 被 Aryan-Railtown 占
- #1393 More insightful error message for bad schema —— 被 CoronRing 占
- #1357 Agent response do not include tool calls. —— 被 CoronRing 占
- #1347 [Epic] Rich media support —— 被 CoronRing 占
- #1343 Thorough Review of Coding Assistant support —— 被 Pooria90 占
- #1266 General Verifier —— 被 Aryan-Railtown 占
- #1265 Implement Naive Human in the loop —— 被 Aryan-Railtown 占
- #1239 Rethinking observability into an events stream —— 被 Amir-R25 占
- #1233 Prebuilt Tools —— 被 Pooria90 占
- #1228 [Feature] Json should store in UTF-16 instead of ASC-II —— 被 CoronRing 占
- #1217 [Feature] [Middleware] Thinking model support — streaming, storage, and round-tripping of reasoning blocks —— 被 CoronRing 占
- #1179 [Docs] Define of keywords —— 被 PR#1566 占
- #1156 [Feature]  Auto populate agent manifest by ingesting system message. —— 被 CoronRing 占
- #1145 [Feature] General Issue for Supported Add Ons —— 被 soulFood5632 占
- #1114 [Docs][Feature] Minor Doc Grammar and content fixes —— 被 CoronRing 占
- #947 [Tech Debt] LLM submodule needs its own loggers —— 被 Aryan-Railtown 占
- #881 [Feature] Add Support for non-google docstring formats —— 被 PR#1452 占
- #486 [Feature] Documentation App —— 被 Aryan-Railtown 占

## 全量总表

| # | 难度 | 状态 | 一句话 |
| --- | --- | --- | --- |
| #486 | 困难 | 🔒被认领 | 文档应用+MCP服务，架构级新子系统，价值高但范围大 |
| #853 | 困难 | 🟡困难 | MCP 协议补全缺失能力，量大且涉及协议设计 |
| #873 | 简单 | 🟢机会 | Context 用法文档前置，纯文档改动，适合入门 |
| #881 | 中等 | 🔒被PR占 | docstring 多格式解析，范围明确需改解析逻辑 |
| #940 | 简单 | 🟢机会 | BYFA/RYFA 教程叙事重构，纯文档改写 |
| #947 | 中等 | 🔒被认领 | LLM 子模块独立 logger 配置，单模块日志改造 |
...
| #1114 | 简单 | 🔒被认领 | 文档语法内容修缺父票，长期跟踪，零散小改 |
| #1131 | 困难 | 🟡困难 | RetrievalRuntime 替换 RAGConfig，架构级重设计 |
| #1142 | 困难 | 🟡困难 | LLM 层缓存式上下文压缩，跨模块且依赖未完成项 |
| #1145 | 困难 | 🔒被认领 | 默认插件批量父票，范围未拆分信息不足 |
| #1150 | 简单 | 🟢机会 | 存储层批量写入 EmbeddedChunk，接口扩展明确 |
| #1156 | 困难 | 🔒被认领 | 用 system message 自动生成 agent manifest，自动化程度需设计决策 |
...
| #1179 | 简单 | 🔒被PR占 | 为文档核心术语定义 glossary 并互链 |
...
| #1217 | 困难 | 🔒被认领 | 全链路思考块支持，跨流式/解析/序列化架构级改造 |
| #1223 | 简单 | 🟢机会 | 给预构建工具打标签以便可视化器区分展示 |
| #1228 | 简单 | 🔒被认领 | JSON 存储改 UTF-16 以下提升人类/代理可读性 |
| #1233 | 困难 | 🔒被认领 | 预构建工具集（内存/文件/TODO/搜索），tracking 票范围大 |
| #1239 | 困难 | 🔒被认领 | 可观测性重构为事件流，架构级替换现有状态导出 |
| #1248 | 中等 | 🟢机会 | 引入 ModelRequest 类型统一模型调用入参 |
| #1251 | 中等 | 🟢机会 | viz 命令 --dir 标志形式化并修复子目录感知 bug |
| #1265 | 困难 | 🔒被认领 | HIL 人机协同作为节点中间件，父票含设计决策 |
| #1266 | 中等 | 🔒被认领 | 通用验证器中间件：callable 校验节点输入并抛异常 |
...
| #1343 | 困难 | 🔒被认领 | 编码助手支持全面评估与改进，研究+实现混合票 |
| #1347 | 困难 | 🔒被认领 | 富媒体支持 Epic，多方向大范围架构工作 |
...
| #1425 | 中等 | 🔒被认领 | 把 verifier 决策结构化为可观测性事件而非日志 |
| #1430 | 中等 | 🔒被认领 | LLM finish_reason 被丢弃，max token 截断时返回空串 |
| #1431 | 中等 | 🔒被认领 | 在 LLM 响应/历史对象中透出 thinking tokens |
| #1433 | 简单 | 🔒被PR占 | 成本计算纳入 cache hit 折扣 token，显著提升准确度 |
| #1435 | 中等 | 🟢机会 | railtracks 导入时全局修改 litellm 参数，应局部化 |
| #1446 | 中等 | 🟢机会 | 新增 context 创建/读写/完成四类可观测事件 |
| #1451 | 中等 | 🔒被认领 | skill 安装从单文件改为目录，元数据从磁盘 SKILL.md 派生 |
| #1454 | 困难 | 🟡困难 | 节点创建时发布静态边检测事件，作为可视化结构图 |
| #1456 | 困难 | 🟡困难 | 重试时切换/降级 LLM 模型的 router 式接口，需设计审批 |
| #1458 | 中等 | 🔒被认领 | from __future__ import annotations 下工具参数 schema 静默退化为 object |
...
| #1474 | 中等 | 🔒被认领 | 为 ticket/PR 设置最长停留时长并自动延期机制，属流程规范类 |
...
| #1538 | 中等 | 🔒被PR占 | 预置中间件未参数化泛型导致 mypy 类型坍缩，根因已定位 |
| #1547 | 简单 | 🔒被PR占 | 文档去模型特定表述（Claude Code、OpenAI 示例），纯文档编辑 |
| #1562 | 中等 | 🔒被认领 | Tool.from_function 把未映射类型静默降级为 object，绕过 #1552 严格校验 |
...
### #1562 [中等|🔒被认领] Tool.from_function silently degrades unmapped parameter types to "object" (bypasses #1552's strict validation)
...
### #1547 [简单|🔒被PR占] Remove/soften model specific instruction in docs
...
### #1538 [中等|🔒被PR占] Type hints collapse when you use a list of pre-built middlewares
...
### #1474 [中等|🔒被认领] We need a ticket assign max duration or PR open duration
